...
### update_label_readmes.py

Updates existing label README files to add Origin Code and Product Class/Type metadata if they're missing. This script fetches the metadata from the TTB COLA Public Registry and updates README files to match the format used by `download_ttb_labels.py`.

**Usage:**
```bash
# Update all label READMEs with missing fields
python3 .github/scripts/update_label_readmes.py
```

**Features:**
- Automatically detects which READMEs are missing Origin Code or Product Class/Type
- Fetches metadata from TTB COLA Public Registry
- Updates README files to match standard format
- Skips files that already have both fields
- Respects TTB server with delays between requests
- Provides detailed progress output and summary statistics
- Stops processing if CAPTCHA is detected

**Example Output:**
```
Found 179 label directories

[1/179] 08182001000002:
  ✓ Added: Origin Code, Product Class/Type
[2/179] 09112001000216:
  ✓ Added: Origin Code, Product Class/Type
...
...
Summary:
  Updated: 176
  Skipped (already had fields): 3
  Not found on TTB page: 0
  Errors: 0
============================================================
```

**Note:** This is a one-time maintenance script. New labels downloaded with `download_ttb_labels.py` will automatically include these fields.

### fix_readme_formatting.py

Fixes formatting issues in label README files to ensure proper blank lines between metadata fields.

**Usage:**
```bash
# Fix formatting in all label READMEs
python3 .github/scripts/fix_readme_formatting.py
```

**Features:**
- Ensures blank line between Issue Date and Origin Code
- Only modifies files that need formatting fixes
- Preserves all content, only adjusts whitespace
...
[1/179] 08182001000002: Fixed formatting
[2/179] 09112001000216: Fixed formatting
...
